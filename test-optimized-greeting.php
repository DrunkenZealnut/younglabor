<?php
// Optimized mode 테스트용 파일
// Legacy 모드를 건드리지 않고 optimized mode만 테스트

// 강제로 optimized mode 설정
$_GET['css_mode'] = 'optimized';

// 모던 부트스트랩 시스템 사용
require_once __DIR__ . '/bootstrap/app.php';

// 페이지 메타
$pageTitle = '이사장 인사말 (Optimized Mode Test) | ' . app_name();
$pageDescription = '사단법인 희망씨 이사장 인사말 - Optimized Mode';
...
<main id="container" role="main">
  <article aria-labelledby="greeting-title" class="max-w-4xl mx-auto px-4 py-10">
    <header class="mb-8">
      <p class="text-sm text-gray-500">About (Optimized Mode)</p>
      <h1 id="greeting-title" class="text-3xl md:text-4xl font-bold text-forest-600">이사장 인사말</h1>
    </header>

    <section aria-labelledby="headline" class="mb-8 rounded-2xl bg-natural-100 gradient-natural p-6 md:p-8">
      <h2 id="headline" class="text-2xl md:text-3xl font-extrabold text-forest-700">희망씨가 희망입니다.</h2>
    </section>
    ...
    <!-- 디버그 정보 -->
    <div style="margin-top: 2rem; padding: 1rem; background: #f0f0f0; border-radius: 0.5rem; font-size: 0.8rem;">
      <h3>Optimized Mode 상태 확인:</h3>
      <p>현재 CSS 모드: <?php echo getCSSMode()->getCurrentMode(); ?></p>
      <p>Optimized 모드 여부: <?php echo getCSSMode()->isOptimizedMode() ? 'Yes' : 'No'; ?></p>
      <p>Critical CSS 크기: <?php 
        require_once __DIR__ . '/includes/critical-css-generator.php';
        $generator = new CriticalCSSGenerator();
        echo round($generator->getCriticalCSSSize() / 1024, 2) . ' KB';
      ?></p>
    </div>
  </article>
</main>
