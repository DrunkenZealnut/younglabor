...
$coverFile = ['purpose'=>'cover','storage_name'=>str_repeat('e', 64),'original_name'=>'cover.webp','mime'=>'image/webp','byte_size'=>24,'sha256'=>str_repeat('f', 64),'width'=>640,'height'=>480,'alt_text'=>'자료 표지'];
if ($repo->replaceFile($resource, $coverFile) !== []) exit(1);
$resourceRows = array_values(array_filter($repo->listPublished('resource', 12, 0), static fn(array $item): bool => $item['slug'] === 'guide'));
if (count($resourceRows) !== 1 || ($resourceRows[0]['file_purpose'] ?? null) !== 'cover') exit(1);
$resourceDetail = $repo->findPublishedBySlug('resource', 'guide');
if (($resourceDetail['file_purpose'] ?? null) !== 'attachment' || (int)($resourceDetail['file_id'] ?? 0) !== (int)$publicFile['id']) exit(1);
...
if (count($files) !== 2 || $repo->findAdminById($resource) !== null || count($repo->listPendingCleanup(10)) !== 2) exit(1);
$pdo->exec(file_get_contents($root . '/database/migrations/20260921_drop_managed_content.sql'));
