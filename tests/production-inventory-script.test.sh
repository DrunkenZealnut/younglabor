...
mkdir -p "$fixture/doc root/admin" "$fixture/doc root/uploads" "$fixture/doc root/data/file" "$fixture/doc root/wp-content/uploads" "$fixture/doc root/wp-content/cache" "$fixture/output"
...
printf '%s\n' 'legacy private file' > "$fixture/doc root/data/file/person.txt"
...
! rg -q 'inventory-secret|private upload|legacy private file|wordpress upload|wordpress cache|data/file|wp-content/(uploads|cache)' "$fixture/output"
