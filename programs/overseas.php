...
<main id="main" role="main" class="flex-1">
  <article class="max-w-5xl mx-auto px-4 py-10">
    ...
    <section class="mb-8" aria-label="해외아동지원사업 이미지 슬라이드">
      ...
      <div id="slider-b12" class="slider">
        <?php foreach ($imgSrcs as $i => $src): ?>
          <img src="<?= h($src) ?>" alt="해외아동지원사업 이미지 <?= $i+1 ?>" class="slide <?= $i===0?'active':'' ?>" loading="lazy">
        <?php endforeach; ?>
      ...
      <script>
        (function(){
          var el=document.getElementById('slider-b12'); if(!el) return;
