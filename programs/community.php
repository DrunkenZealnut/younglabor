...
<main id="main" role="main" class="flex-1">
  <article class="max-w-5xl mx-auto px-4 py-10">
    ...
    <section class="mb-8" aria-label="소통 및 회원사업 이미지 슬라이드">
      <div class="community-slider-container" style="position:relative;border-radius:1rem;overflow:hidden;background:#f8faf9;">
        <?php foreach ($imgSrcs as $i => $src): ?>
          <div class="community-slider-item" style="<?= $i===0?'':'display:none;' ?>position:relative;width:100%;">
            <img src="<?= h($src) ?>" alt="소통 및 회원사업 이미지 <?= $i+1 ?>" style="width:100%;height:auto;display:block;" loading="lazy">
          </div>
        <?php endforeach; ?>
        
        <div class="community-slider-controls" style="position:absolute;top:50%;left:0;right:0;transform:translateY(-50%);display:flex;justify-content:space-between;padding:0 1rem;opacity:0;transition:opacity 0.3s;z-index:10;">
          <button onclick="changeCommunitySlide(-1)" style="background:rgba(0,0,0,0.7);color:white;border:none;border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;font-size:18px;cursor:pointer;transition:all 0.2s;">‹</button>
          <button onclick="changeCommunitySlide(1)" style="background:rgba(0,0,0,0.7);color:white;border:none;border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;font-size:18px;cursor:pointer;transition:all 0.2s;">›</button>
        </div>
      </div>
      
      <style>
        .community-slider-container:hover .community-slider-controls { opacity: 1; }
        .community-slider-controls button:hover { background: rgba(0,0,0,0.9) !important; transform: scale(1.1); }
        @media (hover: none) { .community-slider-controls { opacity: 0.8; } }
      </style>
