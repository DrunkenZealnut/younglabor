...
<main id="main" role="main" class="flex-1">
  <article class="max-w-5xl mx-auto px-4 py-10">
    ...
    <section class="mb-8" aria-label="국내아동지원사업 이미지 슬라이드">
      <div class="domestic-slider-container" style="position:relative;border-radius:1rem;overflow:hidden;background:#f8faf9;">
        <?php foreach ($imgSrcs as $i => $src): ?>
          <div class="domestic-slider-item" style="<?= $i===0?'':'display:none;' ?>position:relative;width:100%;">
            <img src="<?= h($src) ?>" alt="국내아동지원사업 이미지 <?= $i+1 ?>" style="width:100%;height:auto;display:block;" loading="lazy">
          </div>
        <?php endforeach; ?>
        
        <div class="domestic-slider-controls" style="position:absolute;top:50%;left:0;right:0;transform:translateY(-50%);display:flex;justify-content:space-between;padding:0 1rem;opacity:0;transition:opacity 0.3s;z-index:10;">
          <button onclick="changeDomesticSlide(-1)" style="background:rgba(0,0,0,0.7);color:white;border:none;border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;font-size:18px;cursor:pointer;transition:all 0.2s;">‹</button>
          <button onclick="changeDomesticSlide(1)" style="background:rgba(0,0,0,0.7);color:white;border:none;border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;font-size:18px;cursor:pointer;transition:all 0.2s;">›</button>
        </div>
      </div>
      
      <style>
        .domestic-slider-container:hover .domestic-slider-controls { opacity: 1; }
        .domestic-slider-controls button:hover { background: rgba(0,0,0,0.9) !important; transform: scale(1.1); }
        @media (hover: none) { .domestic-slider-controls { opacity: 0.8; } }
      </style>
      
      <script>
        (function() {
          let domesticSlideIndex = 0;
          const domesticSlides = document.querySelectorAll('.domestic-slider-item');
          
          function changeDomesticSlide(direction) {
            domesticSlides[domesticSlideIndex].style.display = 'none';
            domesticSlideIndex = (domesticSlideIndex + direction + domesticSlides.length) % domesticSlides.length;
            domesticSlides[domesticSlideIndex].style.display = 'block';
          }
          
          // 전역 함수로 등록
          window.changeDomesticSlide = changeDomesticSlide;
          
          // 자동 슬라이드
          setInterval(() => changeDomesticSlide(1), 5000);
        })();
      </script>
    </section>
    <?php endif; ?>
