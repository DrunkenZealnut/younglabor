...
try {
    ...
    $count_sql = "SELECT COUNT(*) as total FROM " . get_table_name('posts') . " WHERE board_type = 'gallery' AND $base_where";
...
<main id="main" role="main" class="flex-1">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
      ...
      <?php if ($useCSSVars): ?>
        <h1 class="text-3xl md:text-4xl font-bold" style="<?= $styleManager->getStyleString(['color' => 'forest-600']) ?>">활동 갤러리</h1>
      <?php else: ?>
        <h1 class="text-3xl md:text-4xl font-bold <?= getThemeClass('text', 'primary', '600') ?>">활동 갤러리</h1>
      <?php endif; ?>
      <p class="text-gray-600 mt-2">희망씨의 다양한 활동 사진을 확인하세요.</p>
    </div>
    ...
    <?php else: ?>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <article class="bg-white rounded-lg shadow-sm border border-primary-light hover:border-primary overflow-hidden hover:shadow-md transition-all duration-300">
          <a href="<?= app_url('community/gallery_view.php?wr_id=' . $post['wr_id']) ?>" class="block">
