...
try {
    ...
    $rows = DatabaseManager::select(
        'SELECT wr_id, wr_subject, wr_name, wr_datetime, wr_hit FROM hopec_press WHERE ' . $where_sql . ' ORDER BY wr_datetime DESC LIMIT :limit OFFSET :offset',
        $list_params
    );
} catch (Exception $e) {
    $rows = [];
...
$config = [
    ...
    'enable_search' => true,
    'detail_url' => '/community/press_view.php',
