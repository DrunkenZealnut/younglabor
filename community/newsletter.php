...
$pageDescription = '희망씨의 다양한 소식지를 확인하세요.';
...
try {
    ...
    // where_sql 변수는 더이상 사용하지 않음 (위에서 직접 처리)
    ...
    // 소식지 목록 조회 (단순 쿼리 - 복잡한 GROUP BY 제거)
    ...
    // 각 게시글의 대표 이미지 추출
    foreach ($newsletter_posts as $index => $post) {
        $post['thumbnail'] = '';
        ...
        if (empty($post['thumbnail'])) {
            $post['thumbnail'] = '/theme/natural-green/images/placeholder-newsletter.jpg';
        }
    ...
    $has_prev = false;
    $has_next = false;
    $error_message = is_debug() ? $e->getMessage() : '데이터를 봡0러올 수 없습니다.';
}
?>
...
<main id="main" role="main" class="flex-1">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
      ...
      <?php if ($useCSSVars): ?>
        <h1 class="text-3xl md:text-4xl font-bold" style="<?= $styleManager->getStyleString(['color' => 'forest-600']) ?>">소식지</h1>
      <?php else: ?>
        <h1 class="text-3xl md:text-4xl font-bold <?= getThemeClass('text', 'primary', '600') ?>">소식지</h1>
      <?php endif; ?>
      <p class="text-gray-600 mt-2">희망씨의 다양한 소식지를 확인하세요.</p>
    </div>
          ...
          <a href="<?= app_url('community/newsletter_view.php?wr_id=' . $post['wr_id']) ?>" class="block">
            ...
            <div class="relative h-56 bg-gray-100 overflow-hidden">
              ...
              <img src="<?= h($post['thumbnail']) ?>" 
                   ...
                   loading="lazy"
                   onerror="this.src='/theme/natural-green/images/placeholder-newsletter.jpg'">
            ...
            <!-- 콘텐츠 (패딩 및 간격 조정) -->
            <div class="p-3">
              <h3 class="font-semibold text-gray-900 mb-1 line-clamp-2 leading-tight" title="ID: <?= $post['wr_id'] ?>">
                <?= h($post['wr_subject']) ?>
              </h3>
