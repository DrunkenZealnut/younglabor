...
try {
    // 게시글 정보 조회 및 조회수 증가
    $gallery_item = DatabaseManager::selectOne(
        "SELECT * FROM hopec_posts WHERE wr_id = :wr_id AND board_type = 'gallery'",
        [':wr_id' => $wr_id]
    );
    ...
    DatabaseManager::execute(
        "UPDATE hopec_posts SET wr_hit = wr_hit + 1 WHERE wr_id = :wr_id AND board_type = 'gallery'",
        [':wr_id' => $wr_id]
    );
    ...
    if (!empty($gallery_item['wr_file'])) {
        $attachments = DatabaseManager::select(
            "SELECT * FROM hopec_post_files WHERE board_type = 'gallery' AND wr_id = :wr_id ORDER BY bf_no",
            [':wr_id' => $wr_id]
        );
    }
...
// 페이지 메타 정보 설정
$pageTitle = h($gallery_item['wr_subject']) . ' | 활동 갤러리 | ' . app_name();
$pageDescription = mb_substr(strip_tags($gallery_item['wr_content']), 0, 150) . '...';
...
// 본문에서 이미지 추출
$images = [];
if (preg_match_all('/<img[^>]+src=["\']([^"\']+)["\']/i', $gallery_item['wr_content'], $matches)) {
  ...
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <article class="bg-white rounded-lg shadow-sm border border-primary-light hover:border-primary overflow-hidden transition-all duration-300">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between text-sm text-gray-600">
          <div class="flex items-center space-x-4">
            <span class="flex items-center">
              ...
              <?= h($gallery_item['wr_name']) ?>
            </span>
            <span class="flex items-center">
              <i data-lucide="calendar" class="w-4 h-4 mr-1"></i>
              <?= date('Y년 m월 d일', strtotime($gallery_item['wr_datetime'])) ?>
            </span>
            <span class="flex items-center">
              <i data-lucide="eye" class="w-4 h-4 mr-1"></i>
              조회 <?= number_format($gallery_item['wr_hit'] + 1) ?>
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="px-6 py-8">
        <div class="prose prose-lg max-w-none">
          <?= process_content($gallery_item['wr_content']) ?>
        </div>
      </div>
    ...
    <div class="mt-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      ...
      <div class="flex-1">
        <?php
        // 이전 글
        $prev_gallery = DatabaseManager::selectOne(
            "SELECT wr_id, wr_subject FROM hopec_posts 
             WHERE board_type = 'gallery' AND wr_is_comment = 0 AND wr_datetime < :datetime 
             ORDER BY wr_datetime DESC LIMIT 1",
            [':datetime' => $gallery_item['wr_datetime']]
        );
        
        // 다음 글
        $next_gallery = DatabaseManager::selectOne(
            "SELECT wr_id, wr_subject FROM hopec_posts 
             WHERE board_type = 'gallery' AND wr_is_comment = 0 AND wr_datetime > :datetime 
...
function shareUrl() {
  if (navigator.share) {
    navigator.share({
      title: <?= json_encode($gallery_item['wr_subject']) ?>,
