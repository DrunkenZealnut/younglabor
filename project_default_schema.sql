...
CREATE TABLE `newsletter` (
  ...
  `wr_9` varchar(255) NOT NULL,
  `wr_10` varchar(255) NOT NULL
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='정기 소식지 게시판';
...
(29, 'site_name', '사단법인 희망씨', 'general', NULL, '2025-09-06 03:13:04', '2025-09-09 08:28:24'),
(30, 'site_description', '함께 더불어 사는 삶을 위해', 'general', NULL, '2025-09-06 03:13:04', '2025-09-09 08:28:24'),
...
--
-- Stand-in structure for view `reservation_calendar`
...
--
-- 뷰 구조 `board_posts_view`
--
DROP TABLE IF EXISTS `board_posts_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `board_posts_view`  AS SELECT `atti_notices`.`id` AS `post_id`, 'NOTICE' AS `category_id`, `atti_notices`.`author_id` AS `user_id`, 'admin' AS `author_name`, `atti_notices`.`title` AS `title`, `atti_notices`.`content` AS `content`, `atti_notices`.`views` AS `view_count`, `atti_notices`.`is_important` AS `is_notice`, `atti_notices`.`is_published` AS `is_active`, `atti_notices`.`created_at` AS `created_at`, `atti_notices`.`updated_at` AS `updated_at`, `atti_notices`.`category_type` AS `category_type` FROM `atti_notices` WHERE `atti_notices`.`is_published` = 1 ;
...
-- 뷰 구조 `facility_stats`
--
DROP TABLE IF EXISTS `facility_stats`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `facility_stats`  AS SELECT `f`.`id` AS `id`, `f`.`name` AS `name`, `f`.`capacity` AS `capacity`, count(`r`.`id`) AS `total_reservations`, count(case when `r`.`status` = 'confirmed' then 1 end) AS `confirmed_reservations`, count(case when `r`.`status` = 'pending' then 1 end) AS `pending_reservations`, count(case when `r`.`status` = 'cancelled' then 1 end) AS `cancelled_reservations`, sum(case when `r`.`status` = 'confirmed' then `r`.`total_amount` else 0 end) AS `total_revenue`, avg(case when `r`.`status` = 'confirmed' then timestampdiff(HOUR,`r`.`start_datetime`,`r`.`end_datetime`) end) AS `avg_duration_hours` FROM (`atti_facilities` `f` left join `atti_reservations` `r` on(`f`.`id` = `r`.`facility_id`)) GROUP BY `f`.`id`, `f`.`name`, `f`.`capacity` ;

-- --------------------------------------------------------

--
-- 뷰 구조 `reservation_calendar`
--
DROP TABLE IF EXISTS `reservation_calendar`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `reservation_calendar`  AS SELECT `r`.`id` AS `id`, `r`.`facility_id` AS `facility_id`, `f`.`name` AS `facility_name`, `r`.`organization_name` AS `organization_name`, `r`.`contact_name` AS `contact_name`, `r`.`start_datetime` AS `start_datetime`, `r`.`end_datetime` AS `end_datetime`, `r`.`status` AS `status`, `r`.`total_amount` AS `total_amount`, `r`.`payment_status` AS `payment_status`, to_days(`r`.`end_datetime`) - to_days(`r`.`start_datetime`) + 1 AS `duration_days`, timestampdiff(HOUR,`r`.`start_datetime`,`r`.`end_datetime`) AS `duration_hours` FROM (`atti_reservations` `r` join `atti_facilities` `f` on(`r`.`facility_id` = `f`.`id`)) WHERE `r`.`status` in ('confirmed','pending') ORDER BY `r`.`start_datetime` ASC ;
...
--
-- 테이블의 인덱스 `admin_files`
--
ALTER TABLE `admin_files`
  ADD PRIMARY KEY (`id`);

--
-- 테이블의 인덱스 `atti_activity_logs`
--
ALTER TABLE `atti_activity_logs`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_user_type` (`user_type`),
  ADD KEY `idx_created_at` (`created_at`);

--
-- 테이블의 인덱스 `atti_admins`
--
ALTER TABLE `atti_admins`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `username` (`username`),
  ADD UNIQUE KEY `email` (`email`);

--
-- 테이블의 인덱스 `atti_board_attachments`
--
ALTER TABLE `atti_board_attachments`
  ADD PRIMARY KEY (`attachment_id`),
  ADD KEY `idx_post_id` (`post_id`),
  ADD KEY `idx_file_type` (`file_type`);

--
-- 테이블의 인덱스 `atti_board_categories`
--
ALTER TABLE `atti_board_categories`
  ADD PRIMARY KEY (`category_id`);

--
-- 테이블의 인덱스 `atti_board_comments`
--
ALTER TABLE `atti_board_comments`
...
--
-- 테이블의 인덱스 `atti_board_posts`
--
ALTER TABLE `atti_board_posts`
...
--
-- 테이블의 인덱스 `atti_facilities`
--
ALTER TABLE `atti_facilities`
  ADD PRIMARY KEY (`id`);

--
-- 테이블의 인덱스 `atti_notices`
--
ALTER TABLE `atti_notices`
  ADD PRIMARY KEY (`id`),
  ADD KEY `author_id` (`author_id`),
  ADD KEY `idx_published` (`is_published`),
  ADD KEY `idx_created_at` (`created_at`),
  ADD KEY `idx_notices_published_date` (`is_published`,`created_at`),
  ADD KEY `idx_category_type` (`category_type`);

--
-- 테이블의 인덱스 `atti_notice_attachments`
--
ALTER TABLE `atti_notice_attachments`
  ADD PRIMARY KEY (`attachment_id`),
  ADD KEY `idx_notice_id` (`notice_id`),
  ADD KEY `idx_file_type` (`file_type`);

--
-- 테이블의 인덱스 `atti_programs`
--
ALTER TABLE `atti_programs`
  ADD PRIMARY KEY (`id`),
...
--
-- 테이블의 인덱스 `atti_reservations`
--
ALTER TABLE `atti_reservations`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_facility_datetime` (`facility_id`,`start_datetime`,`end_datetime`),
  ADD KEY `idx_status` (`status`),
  ADD KEY `idx_created_at` (`created_at`),
  ADD KEY `idx_reservations_datetime` (`start_datetime`,`end_datetime`),
  ADD KEY `idx_reservations_facility_status` (`facility_id`,`status`);

--
-- 테이블의 인덱스 `atti_settings`
--
ALTER TABLE `atti_settings`
  ADD PRIMARY KEY (`id`),
...
ALTER TABLE `comments`
  ADD PRIMARY KEY (`comment_id`),
  ADD KEY `idx_post` (`board_type`,`post_id`,`is_deleted`),
  ADD KEY `idx_parent` (`parent_id`),
  ADD KEY `idx_member` (`mb_id`),
  ADD KEY `idx_datetime` (`comment_datetime`),
  ADD KEY `idx_group_order` (`comment_group`,`comment_order`),
  ADD KEY `idx_depth` (`post_id`,`comment_depth`);

--
-- 테이블의 인덱스 `comment_notifications`
--
ALTER TABLE `comment_notifications`
  ADD PRIMARY KEY (`notify_id`),
  ADD KEY `idx_to_member` (`to_mb_id`,`is_read`),
...
--
-- 테이블의 인덱스 `popup_settings`
--
ALTER TABLE `popup_settings`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_active` (`is_active`,`start_date`,`end_date`),
  ADD KEY `idx_priority` (`priority`),
  ADD KEY `idx_popup_active_priority` (`is_active`,`priority`,`start_date`,`end_date`);

--
-- 테이블의 인덱스 `popup_views`
--
ALTER TABLE `popup_views`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_popup_user` (`popup_id`,`user_ip`,`viewed_at`),
  ADD KEY `idx_analytics` (`popup_id`,`viewed_at`,`action`),
  ADD KEY `idx_popup_frequency` (`popup_id`,`user_ip`,`viewed_at`);
...
ALTER TABLE `posts`
  ...
  ADD KEY `idx_board_num` (`board_type`,`wr_num`,`wr_reply`,`wr_parent`),
  ADD KEY `idx_is_comment` (`wr_is_comment`,`wr_id`),
  ADD KEY `idx_datetime` (`board_type`,`wr_datetime`),
  ADD KEY `idx_hit` (`board_type`,`wr_hit`),
  ADD KEY `idx_parent` (`wr_parent`),
  ADD KEY `idx_member` (`mb_id`),
  ADD KEY `idx_comment_status` (`board_type`,`comment_status`);
...
ALTER TABLE `post_files`
  ...
  ADD KEY `idx_board_post` (`board_type`,`wr_id`);
...
--
-- 테이블의 AUTO_INCREMENT `admin_files`
--
ALTER TABLE `admin_files`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_activity_logs`
--
ALTER TABLE `atti_activity_logs`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_admins`
--
ALTER TABLE `atti_admins`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_board_attachments`
--
ALTER TABLE `atti_board_attachments`
  MODIFY `attachment_id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_board_categories`
--
ALTER TABLE `atti_board_categories`
  MODIFY `category_id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_board_comments`
--
ALTER TABLE `atti_board_comments`
  MODIFY `comment_id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_board_posts`
--
ALTER TABLE `atti_board_posts`
  MODIFY `post_id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_facilities`
--
ALTER TABLE `atti_facilities`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_notices`
--
ALTER TABLE `atti_notices`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_notice_attachments`
--
ALTER TABLE `atti_notice_attachments`
  MODIFY `attachment_id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_programs`
--
ALTER TABLE `atti_programs`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_reservations`
--
ALTER TABLE `atti_reservations`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_settings`
--
ALTER TABLE `atti_settings`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- 테이블의 AUTO_INCREMENT `atti_user`
--
ALTER TABLE `atti_user`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `admin_user`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '사용자 ID', AUTO_INCREMENT=2;
...
--
-- 테이블의 AUTO_INCREMENT `comment_notifications`
--
ALTER TABLE `comment_notifications`
...
--
ALTER TABLE `donate`
  MODIFY `wr_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=137;
...
--
-- 테이블의 AUTO_INCREMENT `finance_reports`
--
ALTER TABLE `finance_reports`
...
--
-- 테이블의 AUTO_INCREMENT `hero_sections`
--
ALTER TABLE `hero_sections`
...
--
ALTER TABLE `history`
  MODIFY `wr_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=565;
...
--
-- 테이블의 AUTO_INCREMENT `inquiry_categories`
--
ALTER TABLE `inquiry_categories`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;
...
--
ALTER TABLE `members`
  MODIFY `mb_no` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=404;
...
--
-- 테이블의 AUTO_INCREMENT `newsletter`
--
ALTER TABLE `newsletter`
  MODIFY `wr_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=154;

--
-- 테이블의 AUTO_INCREMENT `notices`
--
ALTER TABLE `notices`
  MODIFY `wr_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=284;

--
-- 테이블의 AUTO_INCREMENT `popup_settings`
--
ALTER TABLE `popup_settings`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- 테이블의 AUTO_INCREMENT `popup_views`
--
ALTER TABLE `popup_views`
...
--
ALTER TABLE `posts`
  MODIFY `wr_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=872;

--
-- 테이블의 AUTO_INCREMENT `post_files`
--
ALTER TABLE `post_files`
  MODIFY `bf_no` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=357;

--
-- 테이블의 AUTO_INCREMENT `press`
--
ALTER TABLE `press`
  MODIFY `wr_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=98;

--
-- 테이블의 AUTO_INCREMENT `resources`
--
ALTER TABLE `resources`
  MODIFY `wr_id` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- 테이블의 AUTO_INCREMENT `theme_presets`
--
ALTER TABLE `theme_presets`
...
--
ALTER TABLE `sms5_book_group`
  MODIFY `bg_no` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- 테이블의 제약사항 `atti_board_attachments`
--
ALTER TABLE `atti_board_attachments`
  ADD CONSTRAINT `atti_board_attachments_ibfk_1` FOREIGN KEY (`post_id`) REFERENCES `atti_board_posts` (`post_id`) ON DELETE CASCADE;

--
-- 테이블의 제약사항 `atti_board_comments`
--
ALTER TABLE `atti_board_comments`
  ADD CONSTRAINT `atti_board_comments_ibfk_1` FOREIGN KEY (`post_id`) REFERENCES `atti_board_posts` (`post_id`) ON DELETE CASCADE,
  ADD CONSTRAINT `atti_board_comments_ibfk_2` FOREIGN KEY (`parent_id`) REFERENCES `atti_board_comments` (`comment_id`) ON DELETE CASCADE;

--
-- 테이블의 제약사항 `atti_board_posts`
--
ALTER TABLE `atti_board_posts`
  ADD CONSTRAINT `atti_board_posts_ibfk_1` FOREIGN KEY (`category_id`) REFERENCES `atti_board_categories` (`category_id`) ON DELETE CASCADE;

--
-- 테이블의 제약사항 `atti_notice_attachments`
--
ALTER TABLE `atti_notice_attachments`
  ADD CONSTRAINT `atti_notice_attachments_ibfk_1` FOREIGN KEY (`notice_id`) REFERENCES `atti_notices` (`id`) ON DELETE CASCADE;

--
-- 테이블의 제약사항 `inquiries`
--
ALTER TABLE `inquiries`
  ADD CONSTRAINT `fk_inquiry_category` FOREIGN KEY (`category_id`) REFERENCES `inquiry_categories` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;

--
-- 테이블의 제약사항 `popup_views`
--
ALTER TABLE `popup_views`
  ADD CONSTRAINT `popup_views_ibfk_1` FOREIGN KEY (`popup_id`) REFERENCES `popup_settings` (`id`) ON DELETE CASCADE;
COMMIT;
