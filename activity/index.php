...
<?php else: ?>
    ...
    <?php foreach ($rows as $row): ?>
        <article class="content-card">
            <?php if (!empty($row['file_id']) && $row['file_purpose'] === 'cover'): ?><a href="<?php echo htmlspecialchars(url('activity/' . $row['slug']), ENT_QUOTES, 'UTF-8'); ?>" class="content-cover"><img src="<?php echo htmlspecialchars(url('media/' . (int)$row['file_id']), ENT_QUOTES, 'UTF-8'); ?>" srcset="<?php echo htmlspecialchars(contentImageSrcset($row), ENT_QUOTES, 'UTF-8'); ?>" sizes="(max-width: 600px) 100vw, (max-width: 900px) 50vw, 33vw" alt="<?php echo htmlspecialchars((string)$row['file_alt_text'], ENT_QUOTES, 'UTF-8'); ?>"<?php if (!empty($row['file_width']) && !empty($row['file_height'])): ?> width="<?php echo (int)$row['file_width']; ?>" height="<?php echo (int)$row['file_height']; ?>"<?php endif; ?> loading="lazy" decoding="async"></a><?php endif; ?>
