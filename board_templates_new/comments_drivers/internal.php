<?php
// Internal comment driver: uses atti_board_comments table defined in create_board_tables.sql
...
if (!function_exists('comments_driver_internal_fetch')) {
    function comments_driver_internal_fetch(PDO $pdo, int $postId, array $options = []): array {
        $stmt = $pdo->prepare(
            ...
            . 'FROM atti_board_comments WHERE post_id = ? AND is_active = 1 ORDER BY comment_id ASC'
        );
        $stmt->execute([$postId]);
...
if (!function_exists('comments_driver_internal_create')) {
    function comments_driver_internal_create(PDO $pdo, array $payload): int {
        $stmt = $pdo->prepare(
            'INSERT INTO atti_board_comments (post_id, user_id, author_name, content, parent_id, is_active, created_at, updated_at) '
            . 'VALUES (?, ?, ?, ?, ?, 1, NOW(), NOW())'
        );
        $stmt->execute([
            (int)$payload['post_id'],
