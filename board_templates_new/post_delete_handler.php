...
try {
    ...
    $sql = "SELECT bp.post_id, bp.title, bp.user_id, bp.author_name, bc.category_type
            FROM atti_board_posts bp 
            LEFT JOIN atti_board_categories bc ON bp.category_id = bc.category_id 
    ...
    // 첨부파일 정보 조회 및 삭제
    $attachment_sql = "SELECT stored_name FROM atti_board_attachments WHERE post_id = ?";
    ...
    // 첨부파일 DB 레코드 삭제
    $delete_attachment_sql = "DELETE FROM atti_board_attachments WHERE post_id = ?";
    ...
    $delete_post_sql = "UPDATE atti_board_posts SET is_active = 0 WHERE post_id = ?";
